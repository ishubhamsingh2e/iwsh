    ...
    def continuous_range(self, point_A: int = 4, point_B: int = 8):
        ...
        if self.resu.multi_hand_landmarks != None:
            for handLandmarks in self.resu.multi_hand_landmarks:
                ...
                pixelCoordinatesLandmark1 = self.draw._normalized_to_pixel_coordinates(
                    normalizedLandmark1.x, normalizedLandmark1.y, self.image_width, self.image_height)
                thumb = pixelCoordinatesLandmark1
                # change the value 4 to change the point of finger acc
                normalizedLandmark2 = handLandmarks.landmark[point_B]
                pixelCoordinatesLandmark2 = self.draw._normalized_to_pixel_coordinates(
                    normalizedLandmark2.x, normalizedLandmark2.y, self.image_width, self.image_height)
